...
function eventTag(a){return a.event_label?`<div><span class="tag" title="기사 제목과 발행 시각에 따른 추정입니다. 표시가 없는 기사의 독립성을 뜻하지 않습니다.">${esc(a.event_label)}</span></div>`:''}
function evidence(articles){return (articles||[]).map(a=>{const url=/^https?:\/\//i.test(a.url||'')?a.url:'';return `<div class="ev">${url?`<a href="${esc(url)}" target="_blank" rel="noopener noreferrer">${esc(a.title)} ↗</a>`:`<span class="ev-title">${esc(a.title)} <span class="tag">${data.status==='DEMO'?'합성':'원문 링크 없음'}</span></span>`}<small>${esc(a.source)} · ${esc(stamp(a.published_at))}</small>${eventTag(a)}</div>`}).join('')}
...
function previousHtml(c){
 const old=c.previous_success;
 if(!old)return '';
 const v=old.company;
 return `<details class="panel monitor previous-result"><summary>이전 결과 보기 · ${esc(stamp(old.generated_at))} KST</summary><p class="body-copy">이전 생성 시점의 자료입니다. 현재 트렌드의 분석이나 현재 건수에 포함하지 않습니다.</p><p class="body-copy">${esc(v.weekly_summary_ko||'')}</p>${main(v).map(briefHtml).join('')}${monitor(v).map(monitorHtml).join('')}</details>`;
}
function render(){const c=companies[companyIndex];
 ...
 if(c.status!=='AVAILABLE'){$('detail-body').innerHTML=overview+`<div class="empty">이 회사의 분석 미완료는 관련 이슈가 없다는 뜻이 아닙니다. 다른 회사의 정상 결과는 회사 선택에서 확인할 수 있습니다.</div>`+previousHtml(c);return;}
 $('detail-body').innerHTML=overview+(main(c).length?`<div class="detail-cols"><div><div class="subhead">Main Brief <span>핵심 브리핑 ${main(c).length}건</span></div>${main(c).map(briefHtml).join('')}</div><div><div class="subhead">Monitoring <span>후속 관찰 ${monitor(c).length}건</span></div>${monitor(c).length?monitor(c).map(monitorHtml).join(''):'<div class="empty">별도 Monitoring 항목이 없습니다.</div>'}</div></div>`:monitor(c).length?`<div class="subhead">Monitoring <span>후속 관찰 ${monitor(c).length}건 · ${c.generation_status==='PARTIAL'?'Main Brief 생성 미완료':'이번 주 Main Brief 없음'}</span></div><div class="company-grid monitoring-grid">${monitor(c).map(monitorHtml).join('')}</div>`:`<div class="empty"><b>${c.generation_status==='PARTIAL'?'Main Brief 생성을 완료하지 못했습니다.':'이번 주 별도 브리핑 항목이 없습니다.'}</b><br>다음 데이터 갱신 시 다시 확인할 수 있습니다.</div>`);
 $('detail-body').insertAdjacentHTML('beforeend',previousHtml(c));
}
